...
def main():
    ...
    if args.pretrained_disp or args.evaluate:
        ...
        if args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, disp_net, 0, None, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, disp_net, pose_exp_net, 0, None, tb_writer)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train(args, train_loader, disp_net, pose_exp_net, optimizer, args.epoch_size, None, tb_writer)
        #logger.train_writer.write(' * Avg Loss : {:.3f}'.format(train_loss))

        # evaluate on validation set
        #logger.reset_valid_bar()
        if args.with_gt:
            errors, error_names = validate_with_gt(args, val_loader, disp_net, epoch, None, tb_writer)
        else:
            errors, error_names = validate_without_gt(args, val_loader, disp_net, pose_exp_net, epoch, None, tb_writer)
